...
#include "client_config_builder.hpp"
...
class client_config_service {
 public:
  void add_or_update_client_config(std::string const& common_name,
                                   autobahn::client_config const& config) {
  ...
  std::tuple<bool, std::string> get_client_config(
    ...
    std::string config =
        autobahn::client_config_builder::build(client_configs_[common_name]);
 ...
 private:
  std::map<std::string, autobahn::client_config> client_configs_;
};
