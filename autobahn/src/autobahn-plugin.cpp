...
OPENVPN_EXPORT int openvpn_plugin_func_v3(
  ...
  std::error_code ec;
  auto [result, string_map] = plugin::handle_event(
      static_cast<plugin_events>(args->type), i_get_args(args->argv),
      i_get_env(args->envp), reinterpret_cast<plugin::handle_t *>(args->handle),
      ec);

  // If we receive an error code, log it and stop processing the event.
  if (ec) {
    std::cout << "[autobahn-plugin] failed to handle event: " << ec.message()
              << std::endl;
    return OPENVPN_PLUGIN_FUNC_ERROR;
  }
...
OPENVPN_EXPORT void openvpn_plugin_close_v1(openvpn_plugin_handle_t handle) {
  ...
  auto h = reinterpret_cast<plugin::handle_t *>(handle);
  std::error_code ec;
  plugin::close(h, ec);
  if (ec) {
    std::cout << "[autobahn-plugin] failure during close: " << ec.message()
              << std::endl;
  }
  delete h;
}
